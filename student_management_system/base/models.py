...
class Assign(models.Model):
    ...
    def __str__(self):
        ...
        cr = Dept.objects.get(id=self.dept_id)
...
class AssignTime(models.Model):
    ...
    period = models.CharField(max_length=50, choices=time_slots, default='12:00 - 12:50')
...
class Attendance(models.Model):
    ...
    date = models.DateField(default='2020, 5, 1')
    status = models.BooleanField(default='True')

    def __str__(self):
        sname = Student.objects.get(name=self.student)
        cname = Class.objects.get(std=self.class_id)
        return '%s : %s' % (sname.name, cname.std)
...
class AttendanceTotal(models.Model):
    ...
    def attendance_class(self):
        ...
        att_class = Attendance.objects.filter(class_id=cr, student=stud, status='True').count()
        return att_class
    ...
    def total_class(self):
        stud = Student.objects.get(name=self.student)
        cr = Class.objects.get(std=self.class_id)
        total_class = Attendance.objects.filter(class_id=cr, student=stud).count()
        return total_class
...
class StudentClasses(models.Model):
    ...
    def get_attendance(self):
        a = AttendanceTotal.objects.get(student=self.student, class_id=self.class_id)
        return a.attendance

test_name = (
    ('Prelims exam', 'Prelims exam'),
    ('Final exam', 'Final exam'),
    ('Assignment', 'Assignment'),
)

class Marks(models.Model):
    studentclass = models.ForeignKey(StudentClasses, on_delete=models.CASCADE)
    name = models.CharField(max_length=50, choices=test_name, default='Final exam')
...
def create_marks(sender, instance, **kwargs):
    if kwargs['created']:
        if hasattr(instance, 'name'):
            ...
            for ass in ass_list:
                try:
                    StudentClasses.objects.get(student=instance, class_id=ass.class_id)
                except StudentClasses.DoesNotExist:
                    sc = StudentClasses(student=instance, class_id=ass.class_id)
                    sc.save()
                    sc.marks_set.create(name='Prelims exam')
                    sc.marks_set.create(name='Final exam')
                    sc.marks_set.create(name='Assignment')
        elif hasattr(instance, 'class_id'):
            stud_list = instance.class_id.student_set.all()
            cr = instance.class_id
            for s in stud_list:
                try:
                    StudentClasses.objects.get(student=s, class_id=cr)
                except StudentClasses.DoesNotExist:
                    sc = StudentClasses(student=s, class_id=cr)
                    sc.save()
                    sc.marks_set.create(name='Prelims exam')
                    sc.marks_set.create(name='Final exam')
                    sc.marks_set.create(name='Assignment')
